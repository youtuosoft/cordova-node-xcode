...
pbxProject.prototype.addResourceFile = function(path, opt, group) {
    ...
    if (!opt.plugin) {
        correctForResourcesPath(file, this, group);
        file.fileRef = this.generateUuid();
    }
...
pbxProject.prototype.removeResourceFile = function(path, opt, group) {
    ...
    correctForResourcesPath(file, this, group.name);
...
pbxProject.prototype.addToFrameworkSearchPaths = function(file) {
    ...
    for (config in configurations) {
        ...
        if (!buildSettings['FRAMEWORK_SEARCH_PATHS'].includes(searchPath)) {
...
pbxProject.prototype.addToLibrarySearchPaths = function(file) {
    ...
    for (config in configurations) {
        ...
        if (searchPath !== '' && !buildSettings['LIBRARY_SEARCH_PATHS'].includes(searchPath)) {
            buildSettings['LIBRARY_SEARCH_PATHS'].push(searchPath);
        }
    }
}
...
pbxProject.prototype.addToHeaderSearchPaths = function(file) {
    ...
    for (config in configurations) {
        ...
        if (typeof file === 'string') {
            ...
            searchPath = searchPathForFile(file, this);
        }
        if (searchPath !== '' && !buildSettings['HEADER_SEARCH_PATHS'].includes(searchPath)) {
...
function correctForResourcesPath(file, project, groupName) {
    return correctForPath(file, project, groupName);
}
